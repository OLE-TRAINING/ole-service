package com.ole.rentalstore.ws.validation;

import org.apache.log4j.Logger;

import com.ole.rentalstore.commons.dto.UserDTO;
import com.ole.rentalstore.commons.dto.UserForPasswordChangeDTO;
import com.ole.rentalstore.commons.error.ErrorMessage;
import com.ole.rentalstore.commons.error.ErrorResponse;
import com.ole.rentalstore.commons.exceptions.bad_request.InvalidUserException;
import com.ole.rentalstore.commons.exceptions.bad_request.NullFieldException;
import com.ole.rentalstore.commons.validation.NullFieldValidator;
import com.ole.rentalstore.commons.validation.UserValidator;

import org.springframework.util.StringUtils;


public class UserRequestValidator {
	
	private static final Logger logger = Logger.getLogger(UserRequestValidator.class);
	
	private UserRequestValidator() {
		
	}

	public static void validateEmailRequest(String email) {
		if (StringUtils.isEmpty(email)) {
			throw new NullFieldException(new ErrorResponse(ErrorMessage.Validation.NULL_OR_EMPTY_FIELD));
		}
		if (!UserValidator.validateEmail(email)) {
			throw new InvalidUserException(new ErrorResponse(ErrorMessage.Validation.INVALID_EMAIL));
		}
	}
	
	public static void validatePasswordRequest(String password) {
		if (StringUtils.isEmpty(password)) {
			throw new NullFieldException(new ErrorResponse(ErrorMessage.Validation.NULL_OR_EMPTY_FIELD));
		}
		if (!UserValidator.validatePassword(password)) {
			throw new InvalidUserException(new ErrorResponse(ErrorMessage.Validation.INVALID_PASSWORD));
		}
	}
	
	public static void validateUsernameRequest(String username) {
		if (StringUtils.isEmpty(username)) {
			throw new NullFieldException(new ErrorResponse(ErrorMessage.Validation.NULL_OR_EMPTY_FIELD));
		}
		if (!UserValidator.validateUsername(username)) {
			throw new InvalidUserException(new ErrorResponse(ErrorMessage.Validation.INVALID_USERNAME));
		}
	}
	
	public static void validateUserPasswordChangeRequest(UserForPasswordChangeDTO user) {
		try {
			NullFieldValidator.checkForNullFields(user);
		} catch (IllegalAccessException e) {
			logger.error(e.getMessage());
		}
		ErrorResponse error = UserValidator.validateUserPasswordChange(user);
		if (error != null) {
			throw new InvalidUserException(error);
		}
	}
	
	public static void validateUserRequest(UserDTO user) {
		try {
			NullFieldValidator.checkForNullFields(user);
		} catch (NullFieldException ex) {
			if (!ex.getFieldName().equals("registrationStatus") && !ex.getFieldName().equals("confirmationToken")) {
				throw new NullFieldException(new ErrorResponse(ErrorMessage.Validation.NULL_OR_EMPTY_FIELD));
			}
		} catch (IllegalAccessException e) {
			logger.error(e.getMessage());
		}
		ErrorResponse error = UserValidator.validateUser(user);
		if (error != null) {
			throw new InvalidUserException(error);
		}
	}
}
